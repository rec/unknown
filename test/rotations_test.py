...
class RotationsTest(unittest.TestCase):
    ...
    def test_three(self):
        self.do_test(ROT3, 0, 0, -1)
        self.do_test(ROT3, 0.20, -1, 0.6)
        self.do_test(ROT3, 0.40, 0, 0.20)
        self.do_test(ROT3, 0.49, 0, 0.47)
        self.do_test(ROT3, 0.50, 0, 0.50)
        self.do_test(ROT3, 0.70, 1, 0.10)
        self.do_test(ROT3, 0.99, 1, 0.97)

    def test_four(self):
        self.do_test(ROT3_OFF, 0, 0, 2 / 3)
        self.do_test(ROT3_OFF, 0.20, 0, 14 / 15)
        self.do_test(ROT3_OFF, 0.40, 1, 0.60)
        self.do_test(ROT3_OFF, 0.49, 1, 0.96)
        self.do_test(ROT3_OFF, 0.50, -1, 0.00)
        self.do_test(ROT3_OFF, 0.70, -1, 4 / 15)
        self.do_test(ROT3_OFF, 0.99, -1, 49 / 75)
